...
export default function AdminDashboard() {
  ...
  const { data: staffList, isLoading: staffLoading } = useGetStaffList({
    query: {
      enabled: !!token,
      queryKey: getGetStaffListQueryKey(),
    },
  });
  ...
  const handleStatusChange = (complaintId: number, status: any) => {
                    ...
                    <div className="space-y-4">
                      {stats.byCategory.map((cat, i) => (
                        <div key={i} className="flex items-center">
                          <div className="w-[100px] text-sm font-medium capitalize truncate">
                            {cat.category}
                          </div>
                          <div className="flex-1 ml-4">
                            <Progress value={stats.total > 0 ? (cat.count / stats.total) * 100 : 0} className="h-2" />
                          </div>
                        ...
                        <div key={i} className="flex items-center">
                          ...
                          <div className="flex-1 ml-4">
                            <Progress 
                              ...
                              className={`h-2 ${
                                pri.priority === 'high' ? '[&>div]:bg-destructive' : 
                                pri.priority === 'medium' ? '[&>div]:bg-orange-500' : ''
                              }`} 
                            />
                          </div>
                          <div className="ml-4 w-8 text-right text-sm text-muted-foreground">
                  ...
                  <div className="flex flex-col sm:flex-row">
                    <div className="flex-1 p-4 md:p-6">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1">
                              <UserPlus className="h-3 w-3" /> {complaint.userName || `User #${complaint.userId}`}
                            </span>
                            <span className="flex items-center gap-1">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2 items-center text-xs">
                        <span className="bg-muted px-2 py-1 rounded capitalize font-medium">
                        ...
                        {complaint.sentiment && (
                          <span className={`px-2 py-1 rounded ${
                            complaint.sentiment === 'negative' || complaint.sentiment === 'frustrated' ? 'bg-destructive/10 text-destructive' : 'bg-muted'
                          }`}>
                    ...
                    <div className="bg-muted/30 p-4 md:p-6 sm:w-64 border-t sm:border-t-0 sm:border-l flex flex-col gap-4">
                      <div className="space-y-2">
                        ...
                        <Select 
                          ...
                          onValueChange={(val) => {
                            if (val !== "unassigned") handleAssign(complaint.id, val);
                          }}
        ...
        <TabsContent value="users" className="space-y-4">
          <Card>
            <div className="rounded-md border">
              <div className="grid grid-cols-[1fr_1fr_100px_150px] gap-4 p-4 font-medium text-sm text-muted-foreground border-b bg-muted/50">
                <div>User</div>
                <div>Email</div>
                <div>Joined</div>
                <div>Role</div>
              ...
              <div className="divide-y">
                {usersLoading ? (
                  ...
                  users?.map((u) => (
                    <div key={u.id} className="grid grid-cols-[1fr_1fr_100px_150px] gap-4 p-4 items-center text-sm hover:bg-muted/30 transition-colors">
                      <div className="flex items-center gap-3 overflow-hidden">
                        <Avatar className="h-8 w-8">
                          <AvatarFallback className={u.role === 'admin' ? 'bg-purple-100 text-purple-700' : u.role === 'staff' ? 'bg-blue-100 text-blue-700' : ''}>
                      ...
                      <div className="text-muted-foreground">{format(new Date(u.createdAt), 'MMM yyyy')}</div>
                      <div>
                        <Select 
                          value={u.role} 
                          onValueChange={(val: any) => handleChangeRole(u.id, val)}
